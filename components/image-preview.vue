  ...
  <ui-dialog :visible="!!previewDate" @close="navigateTo({ params: { date: '' }, query: { mkt } })">
    ...
    <div class="relative grid h-screen w-screen place-items-center of-hidden bg-black/90 text-white p-0">
      ...
      <div class="absolute inset-0 z-1 grid grid-rows-[auto_1fr] pointer-events-none">
        <div class="grid grid-cols-[1fr_2fr_1fr] w-full gap-1 border-b bg-black/30 p-2 shadow backdrop-blur transition-all pointer-events-auto">
          ...
          <div class="flex items-center justify-end gap-1">
            <nuxt-link class="p-1 text-xl hover:bg-black/20 rounded" :to="{ params: { date: '' }, query: { mkt } }">
        ...
        <!-- 左右箭头 -->
        <div class="flex items-center justify-between p-2 md:p-4 pointer-events-auto" @click.self="toggleImageMetaVisible">
          <nuxt-link
            v-if="previewDatePrev" :to="{ params: { date: previewDatePrev }, query: { mkt } }"
            class="border-1 p-3 text-2xl text-white shadow outline-0 backdrop-blur active:bg-black/40 hover:bg-black/30 rounded"
          >
      ...
      <template v-else-if="previewImage">
        ...
        <!-- 底部信息栏 -->
        <div
          class="absolute inset-x-0 z-2 transition-all"
          :class="imageMetaVisible ? 'bottom-0' : 'bottom--100%'"
        >
          <div class="border-t bg-black/60 shadow backdrop-blur">
            <section class="px-4 py-2 text-white md:(px-16 py-8)">
              ...
              <div class="grid grid-cols-3 gap-1 md:(flex flex-wrap items-center)">
                <button
                  ...
                  class="[&[aria-busy]_i]:i-system-uicons-loader flex items-center gap-1 border-1 border-rose-600/70 bg-rose-600/50 p-2 text-xs outline-0 backdrop-blur [&[aria-busy]_i]:animate-spin active:bg-rose-600/70 hover:bg-rose-600/80 rounded"
